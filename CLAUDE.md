...
- `js/audio/pitchListener.js` wraps `getUserMedia` + `AnalyserNode` + a throttled YIN tick loop; screens call `registerCleanup()` (in `main.js`) so the mic is always released when navigating away.
