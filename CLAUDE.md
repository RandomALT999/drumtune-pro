...
- **Pitch detection for tuning: spectral peak scan** (`js/audio/spectralPeak.js`) — Goertzel magnitude scan over the search range, pick the peak nearest the target, plus a **drum-mode pair check**: if the winning peak has a credible peak at ~1/1.6 of its frequency, the winner is the overtone and the lower peak is taken instead. This replaced YIN for hit measurement after a real-device finding: drum overtones are *inharmonic* (first overtone ≈1.6× the fundamental, not 2×), so autocorrelation-family detectors flip between modes and compromise dips — readings jumped between two values ~60 Hz apart hit-to-hit. Verified against synthetic fundamental+overtone mixtures (all cases, <1-cent precision, ~12 ms per analysis).
...
- `js/audio/pitchListener.js` wraps `getUserMedia` + `AnalyserNode`; screens call `registerCleanup()` (in `main.js`) so the mic is always released when navigating away. Two modes: continuous (`onUpdate`, YIN, used by Advanced's FFT view) and **hit-based** (`onHit`) — RMS-onset detection, then ONE spectral-peak measurement per strike on a long (8192-sample) window taken at the end of a ~350 ms capture, past the stick-attack noise but before the ring drifts. Tuning screens use hit mode; a lug that lands within tolerance **locks in permanently** and the active lug auto-advances until all are done.
- In-tune tolerance is **±15 cents** (`IN_TUNE_CENTS` in `tuningMath.js`) — a real-device finding: ±5 was unreachable in practice because head seating, stick placement, and room noise move readings several cents per hit.
