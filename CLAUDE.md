...
- **Pitch detection: custom YIN implementation** (`js/audio/yin.js`), not Pitchy.js — there's no package manager in this dev environment, so the algorithm is vendored directly in vanilla JS instead of depending on a CDN fetch at runtime. Keeps the app fully offline-capable once added to the home screen.
- `js/audio/pitchListener.js` wraps `getUserMedia` + `AnalyserNode` + a throttled YIN tick loop; screens call `registerCleanup()` (in `main.js`) so the mic is always released when navigating away. Two modes: continuous (`onUpdate`, used by Advanced's FFT view) and **hit-based** (`onHit`) — RMS-onset detection measures pitch only from each strike's initial attack window, because a ringing head's pitch drifts as it decays (real-device finding: continuous readings swung wildly between hit and fade-out). Tuning screens use hit mode; a lug that lands within tolerance **locks in permanently** and the active lug auto-advances until all are done.
